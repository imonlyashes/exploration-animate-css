...
(function(win, doc, undefined) {
    ...
    RGraph.SVG.tooltip = function(opt) {
        ...
        if (!opt.text || typeof opt.text === "undefined" || RGraph.SVG.trim(opt.text).length === 0) {
            if (typeof properties.tooltipsOverride !== "function") return;
        }
        //
        ...
        if (RGraph.SVG.isFunction(properties.tooltipsOverride)) {
            ...
            //
            document.body.addEventListener("mouseup", function(e1) {
                obj.removeHighlight();
            }, false);
        ...
        function substitute(original) {
            ...
            var keyReplacementFunction = function() {
                ...
                //
                var colors1 = properties.tooltipsFormattedKeyColors ? properties.tooltipsFormattedKeyColors : properties.colors;
                // Build up the HTML table that becomes the key
                for(var i1 = 0, str1 = []; i1 < specific.values.length; ++i1){
                    var value1 = typeof specific.values === "object" && typeof specific.values[i1] === "number" ? specific.values[i1] : 0;
                    var color1 = colors1[i1];
                    var label1 = typeof properties.tooltipsFormattedKeyLabels === "object" && typeof properties.tooltipsFormattedKeyLabels[i1] === "string" ? properties.tooltipsFormattedKeyLabels[i1] : "";
                    // Chart specific customisations -------------------------
                    if (typeof opt.object.tooltipsFormattedCustom === "function") {
                        // The index/group/sequential index
                        // The index
                        // The colors
                        var ret1 = opt.object.tooltipsFormattedCustom(specific, i1, colors1);
                        if (ret1.continue) continue;
                        if (typeof ret1.label === "string") label1 = ret1.label;
                        if (ret1.color) color1 = ret1.color;
                        if (typeof ret1.value === "number") value1 = ret1.value;
                    }
                    value1 = RGraph.SVG.numberFormat({
                        object: opt.object,
                        num: value1.toFixed(opt.object.properties.tooltipsFormattedDecimals),
                    ...
                    //
                    var borderRadius1 = 0;
                    if (typeof opt.object.properties.tooltipsFormattedKeyColorsShape === "string" && opt.object.properties.tooltipsFormattedKeyColorsShape === "circle") borderRadius1 = "100px";
                    // Facilitate the  property that allows CSS to be added to
                    // the tooltip key color blob
                    var tooltipsFormattedKeyColorsCss1 = "";
                    if (properties.tooltipsFormattedKeyColorsCss) {
                        for(property in properties.tooltipsFormattedKeyColorsCss)if (typeof property === "string") tooltipsFormattedKeyColorsCss1 += "{1}: {2};".format(property.replace(/[A-Z]/, function(match1) {
                            return "-" + match1.toLowerCase();
                        }), String(properties.tooltipsFormattedKeyColorsCss[property]));
                    }
                    str1[i1] = '<tr><td><div class="RGraph_tooltipsFormattedKeyColor" id="RGraph_tooltipsFormattedKeyColor_' + i1 + '" ' + ">Ml</div></td><td>" + '<span id="RGraph_tooltipsFormattedKeyLabel_' + i1 + '">' + label1 + "</span>" + " " + value1 + "</td></tr>";
                    // Now that styles can't be applied inline
                    // (due to the CSP header) then apply them with
                    // JavaScript after a small delay.
                    (function(index1, color1, borderRadius1) {
                        setTimeout(function() {
                            // Align the label left
                            var node1 = document.getElementById("RGraph_tooltipsFormattedKeyLabel_" + index1);
                            if (node1) node1.style.textAlign = "left";
                            // Add some styles to the color blob
                            var colorBlob1 = document.getElementById("RGraph_tooltipsFormattedKeyColor_" + index1);
                            if (colorBlob1) {
                                colorBlob1.style.textAlign = "left";
                                colorBlob1.style.backgroundColor = color1;
                                colorBlob1.style.color = "transparent";
                                colorBlob1.style.pointerEvents = "none";
                                colorBlob1.style.borderRadius = borderRadius1;
                                // Add user specified styles from the
                                // tooltipsFormattedKeyColorsCss property
                                for(var property1 in tooltipsFormattedKeyColorsCss1)if (typeof property1 === "string") colorBlob1.style[property1] = tooltipsFormattedKeyColorsCss1[property1];
                            }
                            // Set the default color for the table to inherit
                            var node1 = document.getElementById("RGraph_tooltipsFormattedKey_table");
                            if (node1) node1.style.color = "inherit";
                        }, 1);
                    })(i1, color1, borderRadius1);
                }
                str1 = str1.join("");
                // Add the key to the tooltip text - replacing the placeholder
                text = text.replace("%{key}", '<table id="RGraph_tooltipsFormattedKey_table">' + str1 + "</table>");
            };
            ...
            if (text.indexOf("%{list}") !== -1) (function() {
                ...
                var str1 = properties.tooltipsFormattedListType === "ol" ? '<ol id="rgraph_formatted_tooltips_list">' : '<ul id="rgraph_formatted_tooltips_list">';
                var items1 = properties.tooltipsFormattedListItems[specific.sequentialIndex];
                if (items1 && items1.length) for(var i1 = 0; i1 < items1.length; ++i1)str1 += "<li>" + items1[i1] + "</li>";
                str1 += properties.tooltipsFormattedListType === "ol" ? "</ol>" : "</ul>";
                // Add the list to the tooltip
                text = text.replace(/%{list}/, str1);
            })();
            ...
            if (text.indexOf("%{table}") !== -1) (function() {
                ...
                if (properties.tooltipsFormattedTableHeaders && properties.tooltipsFormattedTableHeaders.length) {
                    str1 += "<thead><tr>";
                    for(var i1 = 0; i1 < properties.tooltipsFormattedTableHeaders.length; ++i1)str1 += "<th>" + properties.tooltipsFormattedTableHeaders[i1] + "</th>";
                    str1 += "</tr></thead>";
                }
                // Add each row of data
                if (typeof properties.tooltipsFormattedTableData === "object" && !RGraph.SVG.isNull(properties.tooltipsFormattedTableData)) {
                    str1 += "<tbody>";
                    for(var i1 = 0; i1 < properties.tooltipsFormattedTableData[specific.sequentialIndex].length; ++i1){
                        str1 += "<tr>";
                        for(var j1 = 0; j1 < properties.tooltipsFormattedTableData[specific.sequentialIndex][i1].length; ++j1)str1 += "<td>" + String(properties.tooltipsFormattedTableData[specific.sequentialIndex][i1][j1]) + "</td>";
                        str1 += "</tr>";
                    }
                    str1 += "</tbody>";
                }
                // Close the table
                str1 += "</table>";
                text = text.replace(/%{table}/g, str1);
            })();
        ...
        if (opt.object.properties.tooltipsPointer) opt.text += '<div id="RGraph_tooltipsPointer"></div>';
        ...
        if (parseInt(tooltipObj.style.left) < 0) {
            ...
            var pointer = document.getElementById("RGraph_tooltipsPointer");
            if (pointer) (function(pointer1) {
                setTimeout(function() {
                    pointer1.style.left = "calc(10% + 5px)";
                }, 1);
            })(pointer);
        //
        ...
        //
            ...
            (function(pointer1) {
                setTimeout(function() {
                    if (pointer1) pointer1.style.left = "calc(90% - 5px)";
                }, 1);
            })(pointer);
        }
        ...
        tooltipObj.onmousedown = function(e1) {
            e1.stopPropagation();
        };
        // Cancel the mouseup event
        tooltipObj.onmouseup = function(e1) {
            e1.stopPropagation();
        };
        // Cancel the click event
        tooltipObj.onclick = function(e1) {
            if (e1.button == 0) e1.stopPropagation();
        };
...
//# sourceMappingURL=index.ecc5863a.js.map
