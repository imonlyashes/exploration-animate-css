    ...
    RGraph.SVG.Bar = function(conf) {
        ...
        this.set = function(name, value) {
            if (arguments.length === 1 && typeof name === "object") {
                for(i in arguments[0])if (typeof i === "string") {
                    name = ret.name;
                    value = ret.value;
                    this.set(name, value);
                }
        ...
        this.draw = function() {
            ...
            } else // Set the skew transform on the all group if necessary
            ...
            // properties set
            if (properties.errorbars) // Go through the error bars and convert numbers to objects
            for(var i1 = 0; i1 < this.data_seq.length; ++i1){
                if (typeof properties.errorbars[i1] === "undefined" || RGraph.SVG.isNull(properties.errorbars[i1])) properties.errorbars[i1] = {
                    max: null,
                    min: null
                };
                else if (typeof properties.errorbars[i1] === "number") properties.errorbars[i1] = {
                    min: properties.errorbars[i1],
                    max: properties.errorbars[i1]
                };
                else if (typeof properties.errorbars[i1] === "object" && typeof properties.errorbars[i1].max === "undefined") properties.errorbars[i1].max = null;
                else if (typeof properties.errorbars[i1] === "object" && typeof properties.errorbars[i1].min === "undefined") properties.errorbars[i1].min = null;
            }
            //
            ...
            var values = [];
            for(var i1 = 0, max = 0; i1 < this.data.length; ++i1){
                // Errorbars affect the max value
                if (properties.errorbars && typeof properties.errorbars[i1] === "number") var errorbar = properties.errorbars[i1];
                else if (properties.errorbars && typeof properties.errorbars[i1] === "object" && typeof properties.errorbars[i1].max === "number") var errorbar = properties.errorbars[i1].max;
                else var errorbar = 0;
                if (typeof this.data[i1] === "number") values.push(this.data[i1] + errorbar);
                else if (RGraph.SVG.isArray(this.data[i1]) && properties.grouping === "grouped") values.push(RGraph.SVG.arrayMax(this.data[i1]) + errorbar);
                else if (RGraph.SVG.isArray(this.data[i1]) && properties.grouping === "stacked") values.push(RGraph.SVG.arraySum(this.data[i1]) + errorbar);
            }
            ...
            //
            // Set the ymin to zero if it's set mirror
            if (mirrorScale) this.scale = RGraph.SVG.getScale({
                object: this,
                numlabels: properties.yaxisLabelsCount,
                unitsPre: properties.yaxisScaleUnitsPre,
                unitsPost: properties.yaxisScaleUnitsPost,
            ...
            if (properties.xaxisLabels && properties.xaxisLabels.length) {
                if (typeof properties.xaxisLabels === "string") properties.xaxisLabels = RGraph.SVG.arrayPad({
                    array: [],
                ...
                // Label substitution
                //
                for(var i1 = 0; i1 < properties.xaxisLabels.length; ++i1)properties.xaxisLabels[i1] = RGraph.SVG.labelSubstitution({
                    object: this,
            ...
            // Draw the key
            if (properties.key, RGraph.SVG.drawKey) RGraph.SVG.drawKey(this);
            else if (!RGraph.SVG.isNull(properties.key)) alert("The drawKey() function does not exist - have you forgotten to include the key library?");
        ...
        this.drawBars = function() {
            ...
            if (properties.shadow) RGraph.SVG.setShadow({
                object: this,
            ...
            // Go through the bars
            for(var i1 = 0, sequentialIndex = 0; i1 < this.data.length; ++i1, ++sequentialIndex){
                //
                // REGULAR BARS
                //
                if (typeof this.data[i1] === "number") {
                    var outerSegment = (this.graphWidth - properties.marginInnerLeft - properties.marginInnerRight) / this.data.length, height = (Math.abs(this.data[i1]) - Math.abs(this.scale.min)) / (Math.abs(this.scale.max) - Math.abs(this.scale.min)) * this.graphHeight, width = (this.graphWidth - properties.marginInnerLeft - properties.marginInnerRight) / this.data.length - properties.marginInner - properties.marginInner, x = properties.marginLeft + properties.marginInner + properties.marginInnerLeft + outerSegment * i1;
                    // Work out the height and the Y coord of the Bar
                    if (this.scale.min >= 0 && this.scale.max > 0) y = this.getYCoord(this.scale.min) - height;
                    else if (this.scale.min < 0 && this.scale.max > 0) {
                        height = Math.abs(this.data[i1]) / (this.scale.max - this.scale.min) * this.graphHeight;
                        y = this.getYCoord(0) - height;
                        if (this.data[i1] < 0) y = this.getYCoord(0);
                    } else if (this.scale.min < 0 && this.scale.max < 0) {
                        height = (Math.abs(this.data[i1]) - Math.abs(this.scale.max)) / (Math.abs(this.scale.min) - Math.abs(this.scale.max)) * this.graphHeight;
                        y = properties.marginTop;
                    }
                    var rect = RGraph.SVG.create({
                        svg: this.svg,
                        type: "rect",
                        parent: properties.variant === "3d" && this.data[i1] < 0 ? this.threed_xaxis_group : this.svg.all,
                        attr: {
                            ...
                            "data-tooltip": !RGraph.SVG.isNull(properties.tooltips) && properties.tooltips.length ? properties.tooltips[i1] : "",
                            "data-index": i1,
                            "data-sequential-index": sequentialIndex,
                            "data-value": this.data[i1],
                    ...
                    if (!this.coords2[0]) this.coords2[0] = [];
                    this.coords2[0].push({
                        object: this,
                        element: rect,
                        x: parseFloat(rect.getAttribute("x")),
                ...
                // GROUPED BARS
                //
                } else if (RGraph.SVG.isArray(this.data[i1]) && properties.grouping === "grouped") {
                    var outerSegment = (this.graphWidth - properties.marginInnerLeft - properties.marginInnerRight) / this.data.length, innerSegment = outerSegment - 2 * properties.marginInner;
                    // Loop through the group
                    for(var j = 0; j < this.data[i1].length; ++j, ++sequentialIndex){
                        var width = (innerSegment - (this.data[i1].length - 1) * properties.marginInnerGrouped) / this.data[i1].length, x = outerSegment * i1 + properties.marginInner + properties.marginLeft + properties.marginInnerLeft + j * width + (j - 1) * properties.marginInnerGrouped;
                        x = properties.marginLeft + properties.marginInnerLeft + outerSegment * i1 + width * j + properties.marginInner + j * properties.marginInnerGrouped;
                        // Calculate the height
                        // eg 0 -> 10
                        if (this.scale.min === 0 && this.scale.max > this.scale.min) var height = (this.data[i1][j] - this.scale.min) / (this.scale.max - this.scale.min) * this.graphHeight, y = this.getYCoord(0) - height;
                        else if (this.scale.max <= 0 && this.scale.min < this.scale.max) {
                            var height = (this.data[i1][j] - this.scale.max) / (this.scale.max - this.scale.min) * this.graphHeight, y = this.getYCoord(this.scale.max);
                            height = Math.abs(height);
                        // eg 10 -> -10
                        } else if (this.scale.max > 0 && this.scale.min < 0) var height = Math.abs(this.data[i1][j]) / (this.scale.max - this.scale.min) * this.graphHeight, y = this.data[i1][j] < 0 ? this.getYCoord(0) : this.getYCoord(this.data[i1][j]);
                        else if (this.scale.min > 0 && this.scale.max > this.scale.min) var height = Math.abs(this.data[i1][j] - this.scale.min) / (this.scale.max - this.scale.min) * this.graphHeight, y = this.getYCoord(this.scale.min) - height;
                        // Add the rect tag
                        var rect = RGraph.SVG.create({
                            svg: this.svg,
                            parent: properties.variant === "3d" && this.data[i1][j] < 0 ? this.threed_xaxis_group : this.svg.all,
                            type: "rect",
                            attr: {
                                stroke: properties.colorsStroke,
                        ...
                        if (!this.coords2[i1]) this.coords2[i1] = [];
                ...
                // STACKED CHARTS
                //
                } else if (RGraph.SVG.isArray(this.data[i1]) && properties.grouping === "stacked") {
                    ...
                    for(var j = 0; j < this.data[i1].length; ++j, ++sequentialIndex){
                        var height = this.data[i1][j] / (this.max - this.min) * this.graphHeight, width = section - 2 * properties.marginInner, x = properties.marginLeft + properties.marginInnerLeft + i1 * section + properties.marginInner, y = y - height;
                        ...
                        if (j === 0 && properties.shadow) {
                            var fullHeight = RGraph.SVG.arraySum(this.data[i1]) / (this.max - this.min) * this.graphHeight;
                            var rect = RGraph.SVG.create({
                                svg: this.svg,
                                parent: this.svg.all,
                                type: "rect",
                        ...
                        // Draw the errorbar if required
                        if (j === this.data[i1].length - 1) this.drawErrorbar({
                            object: this,
                            element: rect,
                            index: i1,
                            value: this.data[i1][j],
        ...
        this.highlight = function(rect) {
            var x = parseFloat(rect.getAttribute("x")) - 0.5, y = parseFloat(rect.getAttribute("y")) - 0.5, width = parseFloat(rect.getAttribute("width")) + 1, height = parseFloat(rect.getAttribute("height")) + 1;
        ...
        this.parseColors = function() {
            ...
            if (!Object.keys(this.originalColors).length) this.originalColors = {
            ...
            if (colors) for(var i1 = 0; i1 < colors.length; ++i1)colors[i1] = RGraph.SVG.parseColorLinear({
                object: this,
                color: colors[i1]
            });
        ...
        this.drawLabelsAbove = function() {
            ...
            if (properties.labelsAbove) {
                var data_seq = RGraph.SVG.arrayLinearize(this.data), seq = 0, stacked_total = 0;
                for(var i1 = 0; i1 < this.coords.length; ++i1, seq++){
                    var num = typeof this.data[i1] === "number" ? this.data[i1] : data_seq[seq];
                    // If this is a stacked chart then only dothe label
                    // if it's the top segment
                    if (properties.grouping === "stacked") {
                        var indexes = RGraph.SVG.sequentialIndexToGrouped(i1, this.data);
                        var group = indexes[0];
                        var datapiece = indexes[1];
                        if (datapiece !== this.data[group].length - 1) continue;
                        else num = RGraph.SVG.arraySum(this.data[group]);
                    }
                    ...
                    if (properties.labelsAboveSpecific && properties.labelsAboveSpecific.length && (typeof properties.labelsAboveSpecific[seq] === "string" || typeof properties.labelsAboveSpecific[seq] === "number")) str = properties.labelsAboveSpecific[seq];
                    else if (properties.labelsAboveSpecific && properties.labelsAboveSpecific.length && typeof properties.labelsAboveSpecific[seq] !== "string" && typeof properties.labelsAboveSpecific[seq] !== "number") continue;
                    var x = parseFloat(this.coords[i1].element.getAttribute("x")) + parseFloat(this.coords[i1].element.getAttribute("width") / 2) + properties.labelsAboveOffsetx;
                    if (data_seq[i1] >= 0) {
                        var y = parseFloat(this.coords[i1].element.getAttribute("y")) - 7 + properties.labelsAboveOffsety;
                        var valign = properties.labelsAboveValign;
                    } else {
                        var y = parseFloat(this.coords[i1].element.getAttribute("y")) + parseFloat(this.coords[i1].element.getAttribute("height")) + 7 - properties.labelsAboveOffsety;
        ...
        this.drawTop3dFace = function(opt) {
            var rect = opt.rect, arr = [
                parseInt(rect.getAttribute("fill")),
                "rgba(255,255,255,0.7)"
            ], x = parseInt(rect.getAttribute("x")), y = parseInt(rect.getAttribute("y")), w = parseInt(rect.getAttribute("width")), h = parseInt(rect.getAttribute("height")), value = parseFloat(rect.getAttribute("data-value"));
            rect.rgraph_3d_top_face = [];
            for(var i1 = 0; i1 < 2; ++i1){
                var color = i1 === 0 ? rect.getAttribute("fill") : "rgba(255,255,255,0.7)";
                ...
                rect.rgraph_3d_top_face[i1] = face;
            }
        };
        //
        // Draws the top of 3D bars
        ...
        this.drawSide3dFace = function(opt) {
            var rect = opt.rect, arr = [
                parseInt(rect.getAttribute("fill")),
                "rgba(0,0,0,0.3)"
            ], x = parseInt(rect.getAttribute("x")), y = parseInt(rect.getAttribute("y")), w = parseInt(rect.getAttribute("width")), h = parseInt(rect.getAttribute("height"));
            rect.rgraph_3d_side_face = [];
            for(var i1 = 0; i1 < 2; ++i1){
                var color = i1 === 0 ? rect.getAttribute("fill") : "rgba(0,0,0,0.3)";
                var face = RGraph.SVG.create({
                    svg: this.svg,
                    type: "path",
                    parent: properties.variant === "3d" && opt.value < 0 ? this.threed_xaxis_group : this.svg.all,
                ...
                // Store a reference to the rect on the front face of the bar
                rect.rgraph_3d_side_face[i1] = face;
            }
        };
        ...
        this.drawErrorbar = function(opt) {
            var index = opt.index, datapoint = opt.value, linewidth = RGraph.SVG.getErrorbarsLinewidth({
                object: this,
                index: index
                ...
                object: this,
                index: index
            }), element = opt.element, type = opt.type;
            ...
            if (datapoint >= 0) {
                ...
                var x1 = parseFloat(element.getAttribute("x")) + parseFloat(element.getAttribute("width")) / 2, y1 = parseFloat(element.getAttribute("y")) + parseFloat(element.getAttribute("height")), y2 = this.getYCoord(parseFloat(datapoint - Math.abs(max)));
        ...
        //
        this.grow = function() {
            var opt = arguments[0] || {}, frames = opt.frames || 30, frame = 0, obj = this, data = [], height = null, seq = 0;
            //
            // Copy the data
            //
            ...
            var iterate = function() {
                for(var i1 = 0, seq = 0, len = obj.coords.length; i1 < len; ++i1, ++seq){
                    ...
                    // the frame number
                    if (typeof data[i1] === "number") {
                        height = Math.abs(obj.getYCoord(data[i1]) - obj.getYCoord(0));
                        obj.data[i1] = data[i1] * multiplier;
                        ...
                        obj.coords[seq].element.setAttribute("y", data[i1] < 0 ? obj.getYCoord(0) : obj.getYCoord(0) - height);
                        // This upadtes the size of the 3D sides to the bar
                        if (properties.variant === "3d") {
                            // Remove the 3D sides to the bar
                            if (obj.coords[i1].element.rgraph_3d_side_face[0].parentNode) obj.coords[i1].element.rgraph_3d_side_face[0].parentNode.removeChild(obj.coords[i1].element.rgraph_3d_side_face[0]);
                            if (obj.coords[i1].element.rgraph_3d_side_face[1].parentNode) obj.coords[i1].element.rgraph_3d_side_face[1].parentNode.removeChild(obj.coords[i1].element.rgraph_3d_side_face[1]);
                            if (obj.coords[i1].element.rgraph_3d_top_face[0].parentNode) obj.coords[i1].element.rgraph_3d_top_face[0].parentNode.removeChild(obj.coords[i1].element.rgraph_3d_top_face[0]);
                            if (obj.coords[i1].element.rgraph_3d_top_face[1].parentNode) obj.coords[i1].element.rgraph_3d_top_face[1].parentNode.removeChild(obj.coords[i1].element.rgraph_3d_top_face[1]);
                            // Add the 3D sides to the bar (again)
                            obj.drawSide3dFace({
                                rect: obj.coords[i1].element
                            });
                            // Draw the top side of the 3D bar
                            if (properties.grouping === "grouped") obj.drawTop3dFace({
                                rect: obj.coords[i1].element
                            });
                            // Now remove and immediately re-add the front face of
                            // the bar - this is so that the front face appears
                            // above the other sides
                            if (obj.coords[i1].element.parentNode) {
                                var parent = obj.coords[i1].element.parentNode;
                                var node = parent.removeChild(obj.coords[i1].element);
                                parent.appendChild(node);
                            }
                        }
                    } else if (typeof data[i1] === "object") {
                        var accumulativeHeight = 0;
                        for(var j = 0, len2 = data[i1].length; j < len2; ++j, ++seq){
                            height = Math.abs(obj.getYCoord(data[i1][j]) - obj.getYCoord(0));
                            height = multiplier * height;
                            obj.data[i1][j] = data[i1][j] * multiplier;
                            height = Math.round(height);
                            obj.coords[seq].element.setAttribute("height", height);
                            obj.coords[seq].element.setAttribute("y", data[i1][j] < 0 ? obj.getYCoord(0) + accumulativeHeight : obj.getYCoord(0) - height - accumulativeHeight);
                        ...
                        //
                        if (obj.stackedBackfaces[i1]) {
                            obj.stackedBackfaces[i1].setAttribute("height", accumulativeHeight);
                            obj.stackedBackfaces[i1].setAttribute("y", obj.height - properties.marginBottom - accumulativeHeight);
                        }
                        // Decrease seq by one so that it's not incremented twice
                        --seq;
                    }
                }
                if (frame++ < frames) //setTimeout(iterate, frame > 1 ? opt.delay : 200);
                RGraph.SVG.FX.update(iterate);
                else if (opt.callback) opt.callback(obj);
            };
            iterate();
            return this;
        };
        ...
        this.wave = function() {
            ...
            opt.startFrames = [];
            opt.counters = [];
            var framesperbar = opt.frames / 3, frame = -1, callback = opt.callback || function() {};
            for(var i1 = 0, len = this.coords.length; i1 < len; ++i1){
                opt.startFrames[i1] = opt.frames / 2 / (obj.coords.length - 1) * i1;
                opt.counters[i1] = 0;
                // Now zero the height of the bar (and remove the 3D faces)
                this.coords[i1].element.setAttribute("height", 0);
                if (this.coords[i1].element.rgraph_3d_side_face) {
                    var parent = this.coords[i1].element.rgraph_3d_side_face[0].parentNode;
                    parent.removeChild(this.coords[i1].element.rgraph_3d_side_face[0]);
                    parent.removeChild(this.coords[i1].element.rgraph_3d_side_face[1]);
                    parent.removeChild(this.coords[i1].element.rgraph_3d_top_face[0]);
                    parent.removeChild(this.coords[i1].element.rgraph_3d_top_face[1]);
                }
            }
            function iterator() {
                ++frame;
                for(var i1 = 0, len = obj.coords.length; i1 < len; ++i1){
                    var el = obj.coords[i1].element;
                    if (frame > opt.startFrames[i1]) {
                        var originalHeight = el.getAttribute("data-original-height"), height, value = parseFloat(el.getAttribute("data-value"));
                        var height = Math.min((frame - opt.startFrames[i1]) / framesperbar * originalHeight, originalHeight);
                        ...
                        if (properties.grouping === "stacked") {
                            ...
                            var indexes = RGraph.SVG.sequentialIndexToGrouped(seq, obj.data);
                            if (indexes[1] > 0) el.setAttribute("y", parseInt(obj.coords[i1 - 1].element.getAttribute("y")) - height);
                        }
                        if (properties.variant === "3d") {
                            // Add the 3D sides to the bar (again)
                            obj.drawSide3dFace({
                            ...
                            // Draw the top side of the 3D bar
                            if (properties.grouping === "grouped" || properties.grouping === "stacked" && indexes[1] + 1 === obj.data[indexes[0]].length) obj.drawTop3dFace({
                                rect: el,
                                value: el.getAttribute("data-value")
                            });
                        }
                ...
                else RGraph.SVG.FX.update(iterator);
            }
            iterator();
            return this;
        };
        ...
        this.tooltipSubstitutions = function(opt) {
            ...
            return {
                ...
                values: typeof this.data[indexes[0]] === "number" ? [
                    this.data[indexes[0]]
                ] : this.data[indexes[0]]
            };
        };
        ...
        this.tooltipsFormattedCustom = function(specific, index) {
            if (typeof this.data[0] === "object") var label = !RGraph.SVG.isNull(properties.tooltipsFormattedKeyLabels) && typeof properties.tooltipsFormattedKeyLabels === "object" && properties.tooltipsFormattedKeyLabels[index] ? properties.tooltipsFormattedKeyLabels[index] : "";
            else var label = !RGraph.SVG.isNull(properties.tooltipsFormattedKeyLabels) && typeof properties.tooltipsFormattedKeyLabels === "object" && properties.tooltipsFormattedKeyLabels[specific.index] ? properties.tooltipsFormattedKeyLabels[specific.index] : "";
            return {
                label: label
            };
        };
        ...
        this.positionTooltipStatic = function(args) {
            var obj = args.object, e = args.event, tooltip = args.tooltip, index = args.index, svgXY = RGraph.SVG.getSVGXY(obj.svg), coords = this.coords[args.index];
            // Position the tooltip in the X direction
            args.tooltip.style.left = svgXY[0] // The X coordinate of the canvas
            ...
            args.tooltip.style.top = svgXY[1] // The Y coordinate of the canvas
            ...
            if (data_arr[index] < 0) args.tooltip.style.top = parseFloat(args.tooltip.style.top) + coords.height / 2 + "px";
            // If the top of the tooltip is off the top of the page
            // then move the tooltip down
            if (parseFloat(args.tooltip.style.top) < 0) args.tooltip.style.top = parseFloat(args.tooltip.style.top) + 20 + "px";
        };
        //
        // Set the options that the user has provided
        //
        for(i in conf.options)if (typeof i === "string") this.set(i, conf.options[i]);
    };
    return this;
// End module pattern
})(window, document);

//# sourceMappingURL=index.51d2b306.js.map
